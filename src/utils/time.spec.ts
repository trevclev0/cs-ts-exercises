...
describe("getTimeSplit", () => {
  ...
  describe("invalid input", () => {
    ...
    it("throws on too many segments", () => {
      expect(() => getTimeSplit("01:30:45:00")).toThrow("Invalid time format");
    });
    ...
    it("throws when a segment is non-numeric", () => {
      expect(() => getTimeSplit("ab:cd:ef")).toThrow("Invalid time format");
    });

    it("throws when only one segment is non-numeric", () => {
      expect(() => getTimeSplit("01:xx:45")).toThrow("Invalid time format");
    });
    it("throws on partially numeric segments", () => {
      expect(() => getTimeSplit("01:30:45x")).toThrow("Invalid time format");
      expect(() => getTimeSplit("01x:30:45")).toThrow("Invalid time format");
    });
...
describe("timeSplitToSecs", () => {
  ...
  it("converts hours only", () => {
    expect(timeSplitToSecs({ hours: 1, minutes: 0, seconds: 0 })).toBe(3600);
  });
  ...
  it("converts a combined time correctly", () => {
    ...
    expect(timeSplitToSecs({ hours: 1, minutes: 30, seconds: 45 })).toBe(5445);
  });
  ...
  it("handles hours greater than 23", () => {
    expect(timeSplitToSecs({ hours: 25, minutes: 0, seconds: 0 })).toBe(90000);
  });
});
...
describe("timeSplitToSecs / secsToTimeSplit round-trip", () => {
  ...
  it.each([
    0, 45, 5445, 86399, 86400, 90000, 90061,
  ])("preserves seconds modulo one day for %i", (secs: number) => {
    expect(timeSplitToSecs(secsToTimeSplit(secs))).toBe(secs % (24 * 3600));
  });
  ...
  it("round-trips with wrapping for values over 24 hours", () => {
    ...
    expect(secsToTimeSplit(timeSplitToSecs(input))).toEqual(expectedAfterWrap);
  });
});
